...
if 'user_info' not in st.session_state:
    st.session_state.user_info = {'logged_in': False, 'user_name': '', 'quiz_results': None}
...
if not st.session_state.user_info['logged_in']:
    ...
    if st.button("테스트 시작하기 "):
        if name_input.strip():
            st.session_state.user_info['user_name'] = name_input
...
# CASE 2: 퀴즈 진행 (버튼 선택형)
    ...
    if st.session_state.current_step < total_q:
        ...
        # 디자인 팁: 버튼을 세로로 큼직하게 배치
        # 각 버튼을 클릭하면 답변 저장 + 다음 단계로 즉시 이동
    ...
    # 결과 계산
        ...
        st.markdown(f"### {st.session_state.user_info['user_name']}, 당신의 시네마틱 페르소나는... \n## **[{RESULT_DETAILS[res_key]['title']}]**")
        st.image(RESULT_DETAILS[res_key]['image'], width=250)
